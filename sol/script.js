document.addEventListener("DOMContentLoaded", () => {
        ...
        submitLead.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    document.getElementById('waBtn1').addEventListener('click', () => {
                        if (typeof fbq === 'function') fbq('track', 'Contact', { content_name: 'WhatsApp Eliecer' });
                        setTimeout(() => {
                            quizModal.classList.remove('active');
                            if (typeof updateWhatsAppFloatingVisibility === 'function') updateWhatsAppFloatingVisibility();
                        }, 1000);
                    });
                    document.getElementById('waBtn2').addEventListener('click', () => {
                        if (typeof fbq === 'function') fbq('track', 'Contact', { content_name: 'WhatsApp Consultor 2' });
                        setTimeout(() => {
                            quizModal.classList.remove('active');
                            if (typeof updateWhatsAppFloatingVisibility === 'function') updateWhatsAppFloatingVisibility();
                        }, 1000);
    ...
    const waFloatingWidget = document.getElementById('whatsappFloatingWidget');
    function updateWhatsAppFloatingVisibility() {
        if (!waFloatingWidget) return;
        ...
        if (isChatActive || isQuizActive) {
            waFloatingWidget.style.opacity = '0';
            waFloatingWidget.style.pointerEvents = 'none';
            waFloatingWidget.style.transform = 'translateY(20px)';
        } else {
            waFloatingWidget.style.opacity = '1';
            waFloatingWidget.style.pointerEvents = 'auto';
            waFloatingWidget.style.transform = 'translateY(0)';
        }
    }
    
        // Rastrear click en botón flotante de WhatsApp
    const waFloatingLink = document.getElementById('waFloatingLink');
    if (waFloatingLink) {
        waFloatingLink.addEventListener('click', () => {
            if (typeof fbq === 'function') fbq('track', 'Contact', { content_name: 'WhatsApp Boton Flotante' });
        });
    }
